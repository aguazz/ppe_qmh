...
\title{Optional R Practice \\ Quantitative Methods for Humanities}
...
\begin{center}
    \textbf{Optional enrichment for 2026--2027.}\\
...
\begin{center}
    \textbf{Optional Use Instructions}    
\end{center}

\begin{itemize}
    \item If you work through this activity, use the provided R Markdown template (\texttt{.Rmd}). Keep section titles and chunk labels (e.g., \texttt{ex1}, \texttt{ex2}, \dots) unchanged so the file remains reproducible.
    \item Fill in your \texttt{student\_id} in the template using a number. The file should compile ("Knit") to HTML from a clean R session (i.e., it should run from start to finish without manual steps).
    \item If the instructor explicitly asks you to submit this optional practice, submit \textbf{both} (in a zip file):
    \begin{itemize}
