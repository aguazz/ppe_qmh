...
\end{enumerate}
    ...
    \begin{enumerate}
        ...
        \]
        Since each person independently has a \( 1 - q \approx 0.9999864 \) probability of \textbf{not} getting 15 or more correct guesses, the probability that all 1,000 fail is:
        \[ 
        ...
        \item While the probability of finding at least one unusually successful test-taker increases with the sample size, this does not mean that psychic ability actually exists.
        \begin{itemize}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \]
        In this case $P(Z > 1.6393) = \texttt{pnorm(1.6393, lower.tail = FALSE)} \approx 0.0506$.
    ...
    \textbf{Conclusion:} While individual probabilities depend on the normal assumption, the Central Limit Theorem lets us approximate probabilities about sample means. As \(n\) grows, the same deviation from the population mean becomes less likely.
    ...
    %% Problem
    %%%%
    \item \textbf{Is There Majority Support for a Housing Policy?}
    ...
    \begin{enumerate}[label=(\alph*)]
        \item What is the sample proportion \(\hat{p}\)?
        \item State the null and alternative hypotheses for the question: ``is support above \(50\%\)?''
        \item Under the null hypothesis, compute the standard error
        \[
            \mathrm{SE}_0(\hat{p}) = \sqrt{\frac{p_0(1-p_0)}{n}},
        \]
        where \(p_0 = 0.50\).
        \item Compute the test statistic
        \[
            z = \frac{\hat{p} - p_0}{\mathrm{SE}_0(\hat{p})}.
        \]
        \item The one-sided p-value is \(P(Z \geq z)\) for \(Z \sim N(0,1)\). It is approximately \(0.055\). Interpret this p-value.
        \item Compute an approximate \(95\%\) confidence interval using
        \[
    ...
    \ifsolutions \color{blue} \textbf{Solution:} \\
    \begin{enumerate}[label=(\alph*)]
        \item The sample proportion is
        \[
            \hat{p} = \frac{216}{400} = 0.54.
        \]

        \item Since the question is whether support is above a majority threshold,
        \[
        ...
        \item Under the null,
        \[
            \mathrm{SE}_0(\hat{p}) = \sqrt{\frac{0.50(1-0.50)}{400}} = \sqrt{\frac{0.25}{400}} = 0.025.
        \]
        ...
        \item The p-value is approximately \(P(Z \geq 1.6) = 0.055\). If true support were exactly \(50\%\), a survey estimate at least as high as \(54\%\) would occur about \(5.5\%\) of the time by sampling variation. This is suggestive evidence of majority support, but it is just above the conventional \(5\%\) threshold.

        \item The confidence interval uses
        \[
    ...
    %% Problem
    %%%%
    \item \textbf{Do Fact-Checking Prompts Improve Accuracy?}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{lccc}
            \toprule
            \textbf{Group} & \textbf{Sample size} & \textbf{Sample mean} & \textbf{Sample standard deviation} \\
            \midrule
    ...
    \begin{enumerate}[label=(\alph*)]
        \item Compute the estimated difference in means, \(\widehat{\tau} = \bar{Y}_T - \bar{Y}_C\).
        \item State the null and two-sided alternative hypotheses.
        \item Compute the standard error
        \[
            \mathrm{SE}(\widehat{\tau}) \approx \sqrt{\frac{s_T^2}{n_T} + \frac{s_C^2}{n_C}}.
        \]
        \item Compute the test statistic \(z = \widehat{\tau}/\mathrm{SE}(\widehat{\tau})\).
        \item The two-sided p-value is approximately \(0.009\). Interpret it.
        \item Compute an approximate \(95\%\) confidence interval for \(\tau\). Does the interval include \(0\)?
        \item Does statistical significance automatically imply that the effect is substantively important? Explain.
    \end{enumerate}

    \ifsolutions \color{blue} \textbf{Solution:} \\
    \begin{enumerate}[label=(\alph*)]
        \item The estimated difference in means is
        \[
            \widehat{\tau} = 76 - 70 = 6.
        \]
        ...
        \[
            \mathrm{SE}(\widehat{\tau}) \approx \sqrt{\frac{12^2}{64} + \frac{14^2}{64}}
            = \sqrt{2.25 + 3.0625}
            = \sqrt{5.3125}
            \approx 2.30.
        \]
        ...
        \item The two-sided p-value is approximately \(0.009\). If the prompt had no effect on average accuracy, a difference at least as far from zero as \(6\) points would be very unlikely. This is strong evidence against \(H_0\) at the \(5\%\) level.

        \item The approximate \(95\%\) confidence interval is
        \[
            6 \pm 1.96(2.30) \approx 6 \pm 4.51 = [1.49,\ 10.51].
        \]
        The interval does not include \(0\), which is consistent with rejecting \(H_0: \tau = 0\).

        \item No. Statistical significance means the estimated difference is large relative to sampling uncertainty. Substantive importance asks whether a \(6\)-point increase is meaningful for the research question, the cost of the prompt, and the decision context.
    \end{enumerate}
    \fi \color{black}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item What if she plays $10{,}000$ times? Use the Central Limit Theorem. Is this method accurate for the previous part where the gambler played only 10 times?
    ...
    \ifsolutions \color{blue} \textbf{Solution:} \\
    \begin{enumerate}
        \item Let the random variable $X$ represent the profit per bet, which can only take two values. Note that $P(X = 1) = 0.49 = 1- P(X=-1)$. The expected profit per bet is
        \[
        ...
        \]
        Numerically, this can be computed as \(\texttt{1 - pbinom(5, n = 10, p = 0.49)}\).
        ...
        \begin{align*}
            ...
            \approx P\left(Z > \frac{0.02}{\sqrt{0.9996}/\sqrt{10{,}000}}\right) \\
            &\approx P(Z > 2.0004) \approx 0.0227
        \end{align*}
        where the last probability was computed using the R command \texttt{pnorm(2.0004, lower.tail = FALSE)}.
        Thus, after \(10{,}000\) bets, the chance of making a positive profit is only about $2.27\%$.
